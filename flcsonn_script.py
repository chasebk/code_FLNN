...
from model.main.hybrid_flnn import CsoFLNN
from utils.IOUtil import read_dataset_file
from utils.SettingPaper import flcsonn_paras as param_grid
...
if test_type == "normal":           ### For normal test
    ...
    all_model_file_name = "stability_flcsonn"
else:
    pass
...
def train_model(item):
    ...
    cso_paras = {
        "epoch": item["epoch"], "pop_size": item["pop_size"], "mixture_ratio": item["mixture_ratio"],
        "smp": item["smp"], "spc": item["spc"], "cdc": item["cdc"], "srd": item["srd"], "w_minmax": item["w_minmax"],
        "c1": item["c1"], "selected_strategy": item["selected_strategy"]
    }
    md = CsoFLNN(root_base_paras=root_base_paras, root_hybrid_paras=root_hybrid_paras, cso_paras=cso_paras)
    md._running__()
