...
def sla_violate(cpu, ram, broker):
    ...
    resources_predict = np.concatenate((cpu_predict, ram_predict), axis=1)
    number_of_VMs = broker.allocate_VMs(resources_usage=resources_predict)
    number_of_VMs += 2
    cpu_pred_VMs = number_of_VMs * broker.capacity_VM[0]
    ram_pred_VMs = number_of_VMs * broker.capacity_VM[1]

    c = np.array((cpu_actual >= cpu_pred_VMs))
    d = np.array((ram_actual >= ram_pred_VMs))
