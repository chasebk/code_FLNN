...
class BaseCSO(RootAlgo):
    ...
    def _seeking_mode__(self, cat):
        ...
        for candidate in candidate_cats:
            if candidate[self.ID_FIT][self.ID_ERROR] != fit1[self.ID_ERROR]:
                flag_equal = False
                break
        ...
        if flag_equal == True:
            ...
            if self.selected_strategy == 0:                # Best fitness-self
                cat = sorted(candidate_cats, key=lambda cat: cat[self.ID_FIT][self.ID_ERROR])[0]

            elif self.selected_strategy == 1:              # Tournament
                k_way = 4
                idx = sample(range(0, self.smp), k_way)
                cats_k_way = [candidate_cats[_] for _ in idx]
                cat = sorted(cats_k_way, key=lambda cat: cat[self.ID_FIT][self.ID_ERROR])[0]

            elif self.selected_strategy == 2:              ### Roul-wheel selection
                fitness_list = [candidate_cats[u][self.ID_FIT][self.ID_ERROR] for u in range(0, len(candidate_cats))]
    ...
    def _train__(self):
        ...
        for i in range(self.epoch):
            ...
            if cat_best[self.ID_FIT][self.ID_ERROR] > cat_current_best[self.ID_FIT][self.ID_ERROR]:
                cat_best = cat_current_best
            self.loss_train.append(cat_best[self.ID_FIT])
            if self.print_train:
                print("Epoch: {}, [MSE, MAE]: {}".format(i+1, cat_best[self.ID_FIT]))
