...
class BaseDE(RootAlgo):
    ...
    def _greedy_selection__(self, pop_old=None, pop_new=None):
        pop = [pop_new[i] if pop_new[i][self.ID_FIT][self.ID_ERROR] < pop_old[i][self.ID_FIT][self.ID_ERROR]
    ...
    def _train__(self):
        ...
        for i in range(self.epoch):
            ...
            if current_best[self.ID_FIT][self.ID_ERROR] < gbest[self.ID_FIT][self.ID_ERROR]:
