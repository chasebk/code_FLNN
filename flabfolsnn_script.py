...
from model.main.hybrid_flnn import ABfoLSFlnn
from utils.IOUtil import read_dataset_file
from utils.SettingPaper import flabfolsnn_paras as param_grid
...
if test_type == "normal":           ### For normal test
    ...
    all_model_file_name = "stability_flabfolsnn"
else:
    pass
...
def train_model(item):
    ...
    abfols_paras = {
        "epoch": item["epoch"], "pop_size": item["pop_size"], "Ci": item["Ci"], "Ped": item["Ped"],
        "Ns": item["Ns"], "N_minmax": item["N_minmax"]
    }
    md = ABfoLSFlnn(root_base_paras=root_base_paras, root_hybrid_paras=root_hybrid_paras, abfols_paras=abfols_paras)
    md._running__()
